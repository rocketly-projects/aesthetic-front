...
import { ALL_APPTS } from "@/lib/data";
...
export default function AgendaPage() {
  const [anchor, setAnchor] = useState(new Date("2026-05-05"));
  const weekDates = getWeekDates(anchor);
  const todayYMD = "2026-05-05";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button className="border border-line bg-surface rounded-md px-2.5 py-1.5 cursor-pointer hover:bg-bg text-ink transition-colors text-sm" onClick={() => { const d = new Date(anchor); d.setDate(d.getDate() + 7); setAnchor(d); }}>›</button>
        <button className="border border-line bg-surface rounded-md px-3.5 py-1.5 text-xs cursor-pointer hover:bg-bg text-ink transition-colors" onClick={() => setAnchor(new Date("2026-05-05"))}>Hoy</button>
        ...
        <div className="overflow-y-auto" style={{ maxHeight: "calc(100vh - 280px)" }}>
          <div className="grid" style={{ gridTemplateColumns: "60px repeat(7, 1fr)" }}>
            ...
            {weekDates.map((d, di) => {
              ...
              const dayAppts = ALL_APPTS.filter((a) => a.date === ymd);
              return (
                <div key={di} className="border-l border-line relative" style={isToday ? { background: "rgba(122,139,110,.03)" } : undefined}>
                  ...
                  {dayAppts.map((appt) => (
                    <div
                      ...
                      style={{ top: apptTop(appt.time), height: apptHeight(appt.duration), background: statusBg[appt.status], borderLeft: `3px solid ${statusBorder[appt.status]}`, padding: "3px 6px" }}
                    >
                      <div className="text-[11px] font-semibold text-ink leading-tight">{appt.clientName}</div>
                      {apptHeight(appt.duration) > 30 && <div className="text-[10px] text-ink-3 mt-px">{appt.time} · {appt.serviceName}</div>}
                    </div>
                  ))}
                  {isToday && <div className="absolute left-0 right-0 h-0.5 z-20" style={{ top: (10.5 - DAY_START) * HOUR_PX, background: "var(--color-accent)" }} />}
                </div>
              );
            })}
          </div>
